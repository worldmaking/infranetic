...
let neataptic_methods = neataptic.methods;
let Network = neataptic.Network;
let neataptic_architect = neataptic.architect;
...
let neato = {
	...
	mutateOnce(genome) {
		let mutationMethod = pick(this.numericMutation);
	...
	mutateStructuralOnce(genome) {
		...
		let mutationMethod = pick(this.structuralMutationAll);
		genome.mutate(mutationMethod);
	},
